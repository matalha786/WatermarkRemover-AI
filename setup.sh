...
if [ "$CHINA_MODE" == "1" ]; then
    pip install "transformers>=4.50.0" "diffusers>=0.30.0" "peft>=0.17.0" "numpy<2" --no-cache-dir $PIP_MIRROR -q
    ...
    pip install "transformers>=4.50.0" "diffusers>=0.30.0" "peft>=0.17.0" "numpy<2" --no-cache-dir -q
...
if [ "$CHINA_MODE" == "1" ]; then
    pip install --upgrade pydantic typer einops omegaconf easydict yacs "peft>=0.17.0" --no-cache-dir $PIP_MIRROR -q
else
    pip install --upgrade pydantic typer einops omegaconf easydict yacs "peft>=0.17.0" --no-cache-dir -q
fi
